...
● 问题在于：即使用了 @ConditionalOnClass，Spring 在加载配置类时仍然需要解析方法签名，而 preAuthorizeAspect(RedisService
redisService)
...
● Update(
atlas-common\atlas-common-security\src\main\java\com\bubua12\atlas\common\security\config\AtlasSecurityAutoConfiguration.java)
...
原理：
当 RedisService 类不存在时，Spring 不会加载 PreAuthorizeAspectConfiguration 类，也就不会解析其中的方法签名，避免了
