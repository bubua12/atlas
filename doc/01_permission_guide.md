...
`@RequiresPermission` 是本项目（Atlas）中用于控制接口访问权限的核心注解。它基于
AOP（面向切面编程）实现，能够拦截标有该注解的方法，并在执行前验证当前登录用户是否拥有指定的权限标识。
...
3. **检查 AOP 代理**：确保 Controller 类被 Spring 管理（加了 `@RestController`），且方法是 `public`
   的。内部自调用（this.method()）不会触发 AOP。
4. **检查网关传递**：确保网关正确传递了 `Authorization` 头或 `X-User-Id`，且拦截器 `UserContextInterceptor` 正常工作（Debug
   看 ThreadLocal 是否有值）。
